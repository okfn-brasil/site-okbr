...
class ITSEC_Password_Requirements {
	...
	private function handle_profile_update_role( $errors, $user ) {
		...
		foreach ( ITSEC_Lib_Password_Requirements::get_registered() as $code => $requirement ) {
			...
			$message = $validated ? $validated : esc_html__( "The provided password does not meet this site's requirements.", 'it-l10n-ithemes-security-pro' );
	...
	public function render_interstitial( $user ) {
		...
		<p class="submit">
			<input type="submit" name="wp-submit" id="wp-submit" class="button button-primary button-large" value="<?php esc_attr_e( 'Update Password', 'it-l10n-ithemes-security-pro' ); ?>"/>
		</p>
