...
final class Eager_Loading_Decorator implements Repository {
	...
	public function get( $id ) {
		if ( ! is_string( $id ) ) {
			throw new Invalid_Argument_Exception( __( 'Id must be a string.', 'it-l10n-ithemes-security-pro' ) );
		}

		$this->all();

		if ( ! isset( $this->all[ $id ] ) ) {
			throw new User_Group_Not_Found( sprintf( __( "No user group found for '%s'", 'it-l10n-ithemes-security-pro' ), $id ) );
		}
