...
class User_Groups extends \WP_REST_Controller {
	...
	public function get_items_permissions_check( $request ) {
		if ( 'edit' === $request['context'] && ! current_user_can( 'itsec_edit_user_groups' ) ) {
			return new \WP_Error( 'rest_forbidden_context', __( 'Sorry, you are not allowed to edit user groups.', 'it-l10n-ithemes-security-pro' ), [ 'status' => rest_authorization_required_code() ] );
		}

		if ( ! current_user_can( 'itsec_list_user_groups' ) ) {
			return new \WP_Error( 'rest_cannot_view', __( 'Sorry, you are not allowed to list user groups.', 'it-l10n-ithemes-security-pro' ), [ 'status' => rest_authorization_required_code() ] );
		}
	...
	public function get_item_permissions_check( $request ) {
		...
		if ( 'edit' === $request['context'] && ! current_user_can( 'itsec_edit_user_group', $request['id'] ) ) {
			return new \WP_Error( 'rest_forbidden_context', __( 'Sorry, you are not allowed to edit this user group.', 'it-l10n-ithemes-security-pro' ), [ 'status' => rest_authorization_required_code() ] );
		}

		if ( ! current_user_can( 'itsec_read_user_group', $request['id'] ) ) {
			return new \WP_Error( 'rest_cannot_view', __( 'Sorry, you are not allowed to view this user group.', 'it-l10n-ithemes-security-pro' ), [ 'status' => rest_authorization_required_code() ] );
		}
	...
	public function create_item_permissions_check( $request ) {
		if ( ! current_user_can( 'itsec_create_user_groups' ) ) {
			return new \WP_Error( 'rest_cannot_create', __( 'Sorry, you are not allowed to create user groups.', 'it-l10n-ithemes-security-pro' ), [ 'status' => rest_authorization_required_code() ] );
		}
	...
	public function create_item( $request ) {
		try {
			...
			return new \WP_Error( 'internal_server_error', __( 'An unexpected error occurred.', 'it-l10n-ithemes-security-pro' ), [ 'status' => \WP_Http::INTERNAL_SERVER_ERROR ] );
		}
	}
	...
	public function update_item_permissions_check( $request ) {
		...
		if ( ! current_user_can( 'itsec_edit_user_group', $request['id'] ) ) {
			return new \WP_Error( 'rest_cannot_edit', __( 'Sorry, you are not allowed to edit this user group.', 'it-l10n-ithemes-security-pro' ), [ 'status' => rest_authorization_required_code() ] );
		}
	...
	private function handle_duplicate_check( User_Group $user_group ) {
		...
		\ITSEC_Lib::load( 'rest' );
		$error    = new \WP_Error( 'rest_duplicate_user_group', __( 'Another user group with this configuration already exists.', 'it-l10n-ithemes-security-pro' ), [ 'status' => \WP_Http::BAD_REQUEST ] );
	...
	public function delete_item_permissions_check( $request ) {
		...
		if ( ! current_user_can( 'itsec_delete_user_group', $request['id'] ) ) {
			return new \WP_Error( 'rest_cannot_delete', __( 'Sorry, you are not allowed to delete this user group.', 'it-l10n-ithemes-security-pro' ), [ 'status' => rest_authorization_required_code() ] );
		}
			...
			if ( isset( $request->get_url_params()['id'] ) ) {
				...
				if ( $this->repository->has( $request['id'] ) ) {
					throw WP_Error::from_code(
						...
						sprintf( __( 'A user group already exists with the id \'%s\'.', 'it-l10n-ithemes-security-pro' ), $request['id'] ),
			...
			if ( isset( $request['users'] ) ) {
				$user_group->set_users( array_map( static function ( $id ) {
					if ( ! $user = get_userdata( $id ) ) {
						throw WP_Error::from_code( 'rest_user_not_found', sprintf( __( 'No user found for %d.', 'it-l10n-ithemes-security-pro' ), $id ), [ 'status' => \WP_Http::BAD_REQUEST ] );
					}
	...
	protected function check_group_exists( \WP_REST_Request $request ) {
		...
		return new \WP_Error( 'rest_not_found', __( 'Sorry, no user group exists with that id.', 'it-l10n-ithemes-security-pro' ), [ 'status' => \WP_Http::NOT_FOUND ] );
	}
