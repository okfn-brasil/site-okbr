...
class ITSEC_Hide_Backend {
	...
	public function notification_strings() {
		return array(
			'label'       => esc_html__( 'Hide Backend – New Login URL', 'it-l10n-ithemes-security-pro' ),
			'description' => sprintf( esc_html__( '%1$sHide Backend%2$s will notify the chosen recipients whenever the login URL is changed.', 'it-l10n-ithemes-security-pro' ), '<a href="#" data-module-link="hide-backend">', '</a>' ),
			'subject'     => esc_html__( 'WordPress Login Address Changed', 'it-l10n-ithemes-security-pro' ),
			'message'     => esc_html__( 'The login address for {{ $site_title }} has changed. The new login address is {{ $login_url }}. You will be unable to use the old login address.', 'it-l10n-ithemes-security-pro' ),
			'tags'        => array(
				'login_url'  => esc_html__( 'The new login link.', 'it-l10n-ithemes-security-pro' ),
