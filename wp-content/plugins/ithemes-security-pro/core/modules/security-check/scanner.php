...
final class ITSEC_Security_Check_Scanner {
	...
	private static function check_server_ips() {
		...
		if ( $ips ) {
			...
			self::$feedback->add_text( __( 'Identified server IPs to determine loopback requests.', 'it-l10n-ithemes-security-pro' ) );
		}
	}
	...
	private static function do_loopback() {
		...
		if ( is_wp_error( $response ) ) {
			...
			self::$feedback->add_text( sprintf( __( 'Skipping loopback test: %s', 'it-l10n-ithemes-security-pro' ), $response->get_error_message() ) );
		...
		if ( ! ITSEC_Lib_IP_Tools::validate( $ip ) ) {
			self::$feedback->add_section( 'loopback', array( 'status' => 'error' ) );
			self::$feedback->add_text( sprintf( __( 'Invalid IP returned: %s', 'it-l10n-ithemes-security-pro' ), esc_attr( $ip ) ) );
		...
		self::$feedback->add_text( __( 'Identified loopback IP.', 'it-l10n-ithemes-security-pro' ) );
	}
}
