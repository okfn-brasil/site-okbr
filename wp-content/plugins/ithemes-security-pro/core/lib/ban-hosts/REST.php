...
class REST extends \WP_REST_Controller {
	...
	public function get_items_permissions_check( $request ) {
		...
		return new \WP_Error( 'rest_cannot_view', __( 'Sorry, you do not have permission to view bans.', 'it-l10n-ithemes-security-pro' ), [ 'status' => rest_authorization_required_code() ] );
	}
	...
	public function get_item( $request ) {
		...
		if ( ! $ban ) {
			return new \WP_Error( 'itsec_ban_not_found', __( 'No ban found with that id.', 'it-l10n-ithemes-security-pro' ), [ 'status' => \WP_Http::NOT_FOUND ] );
		}
	...
	public function create_item_permissions_check( $request ) {
		...
		return new \WP_Error( 'rest_cannot_create', __( 'Sorry, you do not have permission to create bans.', 'it-l10n-ithemes-security-pro' ), [ 'status' => rest_authorization_required_code() ] );
	}
	...
	public function create_item( $request ) {
		...
		try {
			...
			if ( $ban->matches( \ITSEC_Lib::get_ip() ) ) {
				return new \WP_Error( 'itsec_cannot_ban_self', esc_html__( 'The host matches your current IP and cannot be banned.', 'it-l10n-ithemes-security-pro' ), [ 'status' => \WP_Http::BAD_REQUEST ] );
			}
	...
	public function update_item_permissions_check( $request ) {
		...
		return new \WP_Error( 'rest_cannot_edit', __( 'Sorry, you do not have permission to update bans.', 'it-l10n-ithemes-security-pro' ), [ 'status' => rest_authorization_required_code() ] );
	}
	...
	public function delete_item_permissions_check( $request ) {
		...
		return new \WP_Error( 'rest_cannot_delete', __( 'Sorry, you do not have permission to delete bans.', 'it-l10n-ithemes-security-pro' ), [ 'status' => rest_authorization_required_code() ] );
	}
		...
		foreach ( $filters as $filter ) {
			switch ( $filter ) {
				case Filters::ACTOR_TYPE:
					$params[ $filter ] = [
						'title'       => __( 'Actor Type', 'it-l10n-ithemes-security-pro' ),
						'description' => __( 'The type of actor that created the ban.', 'it-l10n-ithemes-security-pro' ),
						'type'        => 'string',
						'enum'        => \ITSEC_Lib::pluck( $this->actor_factory->get_supported_factories(), 'get_slug' ),
					];
					break;
				case Filters::ACTOR_IDENTIFIER:
					$params[ $filter ] = [
						'title'       => __( 'Actor Identifier', 'it-l10n-ithemes-security-pro' ),
						'description' => __( 'The particular actor that caused the ban.', 'it-l10n-ithemes-security-pro' ),
						'type'        => 'string',
					];
					break;
				case Filters::CREATED_AFTER:
					$params[ $filter ] = [
						'title'       => __( 'Banned After', 'it-l10n-ithemes-security-pro' ),
						'description' => __( 'Limit to bans added after the given date.', 'it-l10n-ithemes-security-pro' ),
						'type'        => 'string',
						'format'      => 'date-time',
					];
					break;
				case Filters::CREATED_BEFORE:
					$params[ $filter ] = [
						'title'       => __( 'Banned Before', 'it-l10n-ithemes-security-pro' ),
						'description' => __( 'Limit to bans added before the given date.', 'it-l10n-ithemes-security-pro' ),
						'type'        => 'string',
						'format'      => 'date-time',
					];
					break;
