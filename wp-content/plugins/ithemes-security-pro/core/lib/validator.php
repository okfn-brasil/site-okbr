	...
	final protected function sanitize_setting( $type, $var, $name, $prevent_save_on_error = true, $trim_value = true, $custom_error = '' ) {
		...
		if ( 'string' === $type ) {
			...
			if ( is_numeric( $this->settings[ $var ] ) ) {
				...
				$error = sprintf( __( 'The %1$s value must be a number.', 'it-l10n-ithemes-security-pro' ), $name );
			}
		} elseif ( 'email' === $type ) {
			...
			if ( $invalid_user_groups ) {
				$error = wp_sprintf( _n( 'The following entry in %1$s is invalid: %2$l', 'The following entries in %1$s are invalid: %2$l', count( $invalid_user_groups ), 'it-l10n-ithemes-security-pro' ), $name, $invalid_user_groups );
			}
		} elseif ( 'user-group' === $type ) {
			$source = ITSEC_Modules::get_container()->get( Matchables_Source::class );

			if ( ! $source->has( $this->settings[ $var ] ) ) {
				$error = sprintf( __( 'The user group selected for %1$s is invalid.', 'it-l10n-ithemes-security-pro' ), $name );
			}
