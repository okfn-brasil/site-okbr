...
final class ITSEC_Lockout {
	...
	public function get_lockout_description() {
		...
		$description .= '</li><li>';
		$description .= sprintf( __( '<strong>Is this computer an authorized host:</strong> %s', 'it-l10n-ithemes-security-pro' ), ITSEC_Lib::is_ip_whitelisted( ITSEC_Lib::get_ip() ) === true ? __( 'yes', 'it-l10n-ithemes-security-pro' ) : __( 'no', 'it-l10n-ithemes-security-pro' ) );
		$description .= '</li></ul>';
	...
	public function filter_entry_for_list_display( $entry, $code, $data ) {
		...
		if ( 'whitelisted-host-triggered-blacklist' === $code ) {
			$entry['description'] = esc_html__( 'Authorized Host Triggered Ban Conditions', 'it-l10n-ithemes-security-pro' );
		} elseif ( 'host-triggered-blacklist' === $code ) {
			$entry['description'] = esc_html__( 'Host Triggered Ban Conditions', 'it-l10n-ithemes-security-pro' );
		} elseif ( 'whitelisted-host-triggered-host-lockout' === $code ) {
			$entry['description'] = esc_html__( 'Authorized Host Triggered Host Lockout', 'it-l10n-ithemes-security-pro' );
			...
			$entry['description'] = esc_html__( 'Authorized Host Triggered User Lockout', 'it-l10n-ithemes-security-pro' );
			...
			$entry['description'] = esc_html__( 'Authorized Host Triggered Username Lockout', 'it-l10n-ithemes-security-pro' );
