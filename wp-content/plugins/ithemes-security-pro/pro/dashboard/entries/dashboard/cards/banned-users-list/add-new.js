...
import { find, mapValues, isObject, isEmpty } from 'lodash';
...
export default function AddNew( {
	...
	const createForm = schema && find( schema.links, ( link ) => link.rel === 'create-form' );
	const createFormSchema = useMemo( () => {
		if ( ! createForm ) {
			return;
		}

		return modifySchemaByUiSchema( createForm.submissionSchema, createForm.submissionSchema.uiSchema || {} );
	}, [ createForm ] );
	const onSubmit = async ( e ) => {
		...
		if ( ban instanceof Error ) {
			if ( ban.code === 'rest_invalid_param' && isObject( ban.data.params ) ) {
				const invalidParams = mapValues( ban.data.params, ( error ) => ( { __errors: [ error ] } ) );
		...
		if ( formElement && formElement.current ) {
			const firstInput = formElement.current.formElement.querySelector( 'input' );
