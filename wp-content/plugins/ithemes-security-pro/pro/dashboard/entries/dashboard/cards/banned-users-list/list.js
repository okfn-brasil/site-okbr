	...
	return (
		<Fragment>
			<th scope="row" className={ classnames(
				...
				`itsec-card-banned-users__ban--actor-type-${ ban.created_by ? ban.created_by.type : 'none' }`,
				ban.created_by && `itsec-card-banned-users__ban--actor-id-${ ban.created_by.id }`,
			) }>
				<span className="itsec-card-banned-users__bans-label">{ ban.label }</span>
				{ ban.created_at && (
...
function DetailRender( { master: ban } ) {
	...
	const links = flatten( Object.values( get( ban, '_links', {} ) ) ).filter( ( link ) => link.media === 'text/html' );
	const onSave = async () => {
	...
	return (
		<div className="itsec-card-banned-users__ban">
			<div className="itsec-card-banned-users__ban-top">
				<dl>
					<dt>{ __( 'Host', 'it-l10n-ithemes-security-pro' ) }</dt>
					<dd>{ ban.label }</dd>
					<dt>{ __( 'Time', 'it-l10n-ithemes-security-pro' ) }</dt>
					<dd>{ ban.created_at && dateI18n( 'M d, Y g:i A', ban.created_at ) }</dd>
					<dt>{ __( 'Source', 'it-l10n-ithemes-security-pro' ) }</dt>
					<dd>{ ban.created_by && ban.created_by.label }</dd>
					{ ! canEdit && (
						<>
							<dt>{ __( 'Notes', 'it-l10n-ithemes-security-pro' ) }</dt>
							<dd>{ ban.comment }</dd>
						</>
					) }
				</dl>
				...
				<ul className="itsec-card-banned-users__ban-actions">
					{ getTargetHint( ban, 'allow', false ).includes( 'DELETE' ) && (
						<li>
							<Button
								isLink
								isBusy={ isDeleting }
								...
								icon="dismiss"
							>
								{ __( 'Remove Ban', 'it-l10n-ithemes-security-pro' ) }
							</Button>
						</li>
					) }
			...
			{ canEdit && (
				<TextareaControl
					...
					label={ __( 'Notes', 'it-l10n-ithemes-security-pro' ) }
					value={ comment }
					onChange={ setComment }
...
export default function List( { isSmall, onSelect, selected } ) {
	...
	const { bans, hasNext, isQuerying } = useSelect( ( select ) => ( {
		...
		hasNext: !! select( 'ithemes-security/bans' ).getQueryHeaderLink( 'main', 'next' ),
	...
	return (
		...
		>
			<thead>
				<tr>
					<th className="itsec-card-banned-users__bans--column-label">{ __( 'Host', 'it-l10n-ithemes-security-pro' ) }</th>
					<th className="itsec-card-banned-users__bans--column-comment">{ __( 'Notes', 'it-l10n-ithemes-security-pro' ) }</th>
				</tr>
			</thead>
		</MasterDetail>
	);
